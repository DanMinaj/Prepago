...
class ReportABug extends Model{
	...
	public function reply($reply)
	{
		...
		if(strlen($this->responses) > 0) $responses = unserialize($this->responses);
	...
	public function getCustomerAttribute()
	{
		...
		try {
			...
			if(!$customer) {
				$customer = Customer::where('username', ReportABug::decipherUsername($this->apt_number . $this->apt_building))->first();
			}
