...
class Customer extends Eloquent{
	...
	public static function getPendingCustomers($scheme_id = 0)
	{
		...
		return Customer::pendingShutOff()
		->whereRaw('((customers.status = 1 OR customers.simulator > 0) AND ev_owner = 0)')
		->where('customers.scheme_number', '=', $scheme_id)
		->orderBy('balance', 'asc')
		->get();
	...
	public function confirmsms($message, $charge = 0, $ret = false) 
	{
		try {
			...
			$arr = array('messagetext' => $message, 'senderid' => 'Repliable', 'recipients' => [$this->mobile_number]);
		...
		try {
			...
			switch($key) 
			{
				...
				case "mobile_number":
					...
					$customers = Customer::activeScheme()->orderBy('customers.id', 'DESC')->get();
...
SnugZone.
	...
	*/
	...
	public static function openAccount($data) 
	{
		...
		*/
		$customer = Customer::where('status', '=', 0)->first();
		if(!$customer)
		...
		$customersUsingMeter = Customer::where('meter_ID', $meter_ID)->get();
	...
	public function getAddressAttribute()
	{
		...
		try {
			...
			if($address->city == 'Dublin') $address->city = '';
