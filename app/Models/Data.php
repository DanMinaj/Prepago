...
class Data extends Eloquent{
	...
	public static function backupDB()
	{
		...
		exec("(mysqldump -u" . Data::$username . " -p" . Data::$password . " prepago > $backupDir) 2>&1", $output, $result);
		return $backupName;
	}
	...
	public static function backupTable($table_name)
	{
		...
		$backupName = Data::getTableBackupName($table_name);
		$backupDir = "/var/www/app/storage/backups/individual_tables/$table_name/$backupName";
		echo $backupDir;
		exec("(mysqldump -u" . Data::$username . " -p" . Data::$password . " prepago $table_name > $backupDir) 2>&1", $output, $result);
		return $backupName;
	}
	...
	public static function getDatabaseTables()
	{
		...
		foreach($tables as $t) {
			...
			$t->backups = Data::getFiles($tableDir);
			$t->size = Data::getTableSize($t->Tables_in_prepago);
			...
			if(file_exists($directory) && substr_count($directory,"..") == 1) {
				...
				try {
					...
					if($database == 'prepago_debug') {
						exec("(mysql -u" . Data::$username . " -p" . Data::$password . " prepago_debug < $directory)", $output, $result);
						echo "Restored $table_name to prepago_debug";
					}
					else {
						exec("(mysql -u" . Data::$username . " -p" . Data::$password . " prepago < $directory)", $output, $result);
