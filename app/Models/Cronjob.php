...
class Cronjob extends Model
{
    ...
    {
        return self::where('name', $this->name)->orderBy('time', 'ASC')->get();
    }

    public function getRan()
    {
        return self::where('name', $this->name)->where('ran_today', 1)->count();
    }

    public function getAvgRunTime()
    {
        return number_format(self::where('name', $this->name)->avg('run_time'), 6);
    }
