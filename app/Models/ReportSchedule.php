...
class ReportSchedule extends Model
{
    ...
    {
        $next_run_id = self::orderBy('run_id', 'DESC')->first();
        if (! $next_run_id) {
            $next_run_id = 1;
        } else {
    ...
    public function getProgress()
    {
        ...
        return Response::json([
            ...
            'dl' => 'https://prepagoplatform.com/'.str_replace('/var/www/html/', '', self::$path)
    ...
    {
        if (! $completed) {
            return self::where('run_id', $this->run_id)->orderBy('id', 'ASC')->get();
        } else {
            return self::where('run_id', $this->run_id)
    ...
    public function markComplete()
    {
        ...
        $path = self::$path.'/'.$run_data->folder;
