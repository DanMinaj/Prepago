...
class Chart
{
    ...
    public static function apexFromOptions($options)
    {
        ...
        $ret .= "var " . $options['div'] . "_data = " . Chart::json_encode_advanced($options) . ";\n\n";
	...
	public static function apexDonutChart($div, $data)
	{
		...
		return Chart::apexFromOptions($options);
			...
			foreach($extra as $k => $v) {
				...
				if(!isset($options[$k])) {
					...
					if(is_array($old) && is_array($new)) {		
						 foreach( Chart::arrayRecursiveDiff($new, $old) as $j => $diff) {
							$options[$k][$j] = $diff;
	...
	**/
    ...
    private static function json_encode_advanced(array $arr, $sequential_keys = false, $quotes = false, $beautiful_json = false)
    {
        ...
        $output = Chart::isAssoc($arr) ? "{" : "[";
        $count = 0;
        foreach ($arr as $key => $value)
        {

            if (Chart::isAssoc($arr) || (!Chart::isAssoc($arr) && $sequential_keys == true))
            {
            ...
            if (is_array($value))
            {
                $output .= Chart::json_encode_advanced($value, $sequential_keys, $quotes, $beautiful_json);
            }
	...
	public static function arrayRecursiveDiff($aArray1, $aArray2) { 
		...
		foreach ($aArray1 as $mKey => $mValue) { 
			if (array_key_exists($mKey, $aArray2)) { 
				if (is_array($mValue)) { 
					$aRecursiveDiff = Chart::arrayRecursiveDiff($mValue, $aArray2[$mKey]); 
