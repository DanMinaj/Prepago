...
class Paypal
{
    ...
    {	
		$username = Paypal::$user_acc;
		$password = Paypal::$pw_acc;
		$signature = Paypal::$sig_acc;
		$client = Paypal::$client_acc;
		$secret = Paypal::$secret_acc;
		$this->account = $account;
		
		if($account == 'noreply') {
			$username = Paypal::$user_noreply;
			$password = Paypal::$pw_noreply;
			$signature = Paypal::$sig_noreply;
			$client = Paypal::$client_noreply;
			$secret = Paypal::$secret_noreply;
		}
     ...
     * @return array
     */
     public function call($method, array $params = array(), $account = 'noreply')
    {
		...
		if($account == 'noreply') {
			...
			$fields = array_merge(
				[
					...
					'USER'      => Paypal::$user_noreply,
					'PWD'       => Paypal::$pw_noreply,
					'SIGNATURE' => Paypal::$sig_noreply,
				],
				$params
			);
	...
	**/
	public function getDisputes($from)
	{
		...
		$token2 = Paypal::getToken('accounts');
	...
	**/
	public static function getToken($account = 'accounts')
	{
		...
		if($account == 'accounts') {
			$liveClientID = Paypal::$client_acc;
			$liveSecret = Paypal::$secret_acc;
		} 
		
		if($account == 'noreply') {
			$liveClientID = Paypal::$client_noreply;
			$liveSecret = Paypal::$secret_noreply;
		}
	...
	{	
		$accounts = new \PayPal\Rest\ApiContext(
			new \PayPal\Auth\OAuthTokenCredential(
				Paypal::$client_acc,     // ClientID
				Paypal::$secret_acc      // ClientSecret
			)
		);
	...
	public static function noreplyContext()
	{
		$noreply = new \PayPal\Rest\ApiContext(
			new \PayPal\Auth\OAuthTokenCredential(
				Paypal::$client_noreply,     // ClientID
				Paypal::$secret_noreply      // ClientSecret
			)
		);
