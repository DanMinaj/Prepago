...
class CSVController extends BaseController {
	...
	public function customer_topup_history_by_search($search_key)
	{
		...
		foreach($customers as $customer)
		{
			...
			$p1 = PaymentStorage::where('customer_id', $customer->id)->whereBetween('time_date', array($from, $to))->get(['customer_id', 'time_date', 'amount', 'currency_code']);
			foreach($p1 as $p1_entry)
			$paymentsCollection->push($p1_entry);
				
			$p2 = TemporaryPayments::where('customer_id', $customer->id)->whereBetween('time_date', array($from, $to))->get(['customer_id', 'time_date', 'amount', 'currency_code']);
	...
	public function customer_topup_history_search_by_date($to, $from)
	{
		...
		$payments = PaymentStorage::where('scheme_number', Auth::user()->scheme_number)->whereBetween('time_date', array($from, $to))->orderBy('time_date', 'DESC')->get(['ref_number', 'customer_id', 'time_date', 'amount', 'currency_code', 'acceptor_name_location',
