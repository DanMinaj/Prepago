...
class AccountController extends BaseController {
	...
	public function close_account_search()
	{
		...
		$this->layout->page = View::make('home/close_account_view', array('customers' => $customers));
	}
	...
	{
		$customers = Customer::where('status', '=', 1)->where('scheme_number', '=', Auth::user()->scheme_number)->get();
		$this->layout->page = View::make('home/multiple_close_view', array('customers' => $customers));
	}
	...
	public function shut_offs()
	{
		...
		if(!file_exists($shut_off_file)) {
			...
			foreach($lines as $l) {
				...
				if($customer) {
					if($customer->balance > 0.00) $shut_offs[$dhm->meter_ID]->style = "background-color: #c9eec9;";
					else $shut_offs[$dhm->meter_ID]->style = "background-color: #ffd0ce;";
				} else {
		...
		else 
		{
			...
			foreach($dhm as $d) {
				...
				if($customer) {
					if($customer->balance > 0.00) $shut_offs[$d->meter_ID]->style = "background-color: #c9eec9;";
					else $shut_offs[$d->meter_ID]->style = "background-color: #ffd0ce;";
				} else {
