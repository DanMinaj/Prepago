...
class SchemeController extends BaseController {
    ...
    public function index()
    {
        ...
        $this->layout->page = View::make('schemes/index', array('schemes' => $schemes));
    }
	...
	public function scheme_settings_manage_meter_save() 
	{
		...
		try { 
			...
			foreach(Input::all() as $key => $value) {
				...
				if($key == 'pmd_id' || $key == 'o_meter_sixteen' || $key == 'meter_sixteen' || $key == 'meter_eight' || $key == 'scu_sixteen' || $key == 'scu_eight' || $key == 'o_scu_number' || $key == 'o_meter_number') continue;	
				if($key == 'name')
					continue;
