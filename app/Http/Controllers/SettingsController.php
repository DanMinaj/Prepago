...
class SettingsController extends BaseController
{
    ...
    public function save_sms_message()
    {
        ...
        if ($number == 1) {
            Scheme::where('scheme_number', '=', Auth::user()->scheme_number)->update(['balance_message' => $text]);
        }

        if ($number == 2) {
            Scheme::where('scheme_number', '=', Auth::user()->scheme_number)->update(['IOU_message' => $text]);
        }

        if ($number == 3) {
            Scheme::where('scheme_number', '=', Auth::user()->scheme_number)->update(['IOU_extra_message' => $text]);
        }

        if ($number == 4) {
            Scheme::where('scheme_number', '=', Auth::user()->scheme_number)->update(['IOU_denied_message' => $text]);
        }

        if ($number == 5) {
            Scheme::where('scheme_number', '=', Auth::user()->scheme_number)->update(['rates_message' => $text]);
        }

        if ($number == 6) {
            Scheme::where('scheme_number', '=', Auth::user()->scheme_number)->update(['shut_off_message' => $text]);
        }

        if ($number == 7) {
            Scheme::where('scheme_number', '=', Auth::user()->scheme_number)->update(['shut_off_warning_message' => $text]);
        }

        if ($number == 8) {
            Scheme::where('scheme_number', '=', Auth::user()->scheme_number)->update(['credit_warning_message' => $text]);
        }

        if ($number == 9) {
            Scheme::where('scheme_number', '=', Auth::user()->scheme_number)->update(['barcode_message' => $text]);
        }
