...
class InstallerController extends BaseController {
	...
	//print_r($installed_units);exit;	
		$this->layout->page = View::make('dashboard.index')->with(array('installed_units' => $installed_units, 'searched' => $searched));
	}
	...
	public function access_control()
	{
		...
		$this->layout->page = View::make('dashboard/access_control', array(
			'is_installer' => true,
			'customers' => $customers,
			'schemes' => $schemes,
			'baseURL' => $baseURL
	...
	public function addUnits($type = "default")
	{
		...
		if($last_installed_unit){
			$this->layout->page = View::make('dashboard.addUnits')->with(array(
					'is_ev'					=> $isEV,
					'type' 					=> $type, 
			...
			$this->layout->page = View::make('dashboard.addUnitsBlank')->with(array(
					'is_ev'			=> $isEV,
					'type' 			=> $type, 
					'schemeStreet2' => $schemeStreet2,
					'dataLoggers'	=> $dataLoggers
	...
	public function testUnit($unitID)
	{
		...
		$this->layout->page = View::make('dashboard.test-unit', array('unitID' => $unitID, 'unit' => $unit));
	}
	...
	public function complete_install()
	{
		...
		PermanentMeterData::where('ID', '=', $unitID)->update(array('installation_confirmed' => 1));
	...
	{
		$unitID = Input::get('unitID');
		PermanentMeterData::where('ID', '=', $unitID)->update(array('installation_confirmed' => 0));
		return Redirect::to('prepago_installer');
	}
	...
	public function editUnit($unitID)
	{
		...
		$mbusAddressSCU = ''; $mbusAddressMeter = '';
		$SCUReady = false; $MeterReady = false;
		if($unit)
		if ($unit['scu_type'] == 'm'){
	...
	public function editAddressTranslation($digit)
	{
		...
		$this->layout->page = View::make('dashboard.editAddressTranslation', array('eight' => $eight, 'sixteen' => $sixteen));
	}
	...
	public function editSubmitAddressTranslation($digit)
	{
		...
		if($addresstranslation)
		{
			...
			DB::table('mbus_address_translations')->where('8digit', $old_eight)->update(array('8digit' => $new_eight, '16digit' => $new_sixteen));
