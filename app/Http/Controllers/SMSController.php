...
class SMSController extends BaseController {	
	...
	public function message_to_number($scheme_number, $sms_password, $number, $message)
	{
		...
		$scheme = Scheme::where('scheme_number', '=', $scheme_number)->get(array('sms_password'))->first();
	...
	public function message_to_meter($scheme_number, $sms_password, $number, $message, $customer_id)
	{
		/* Check if the scheme_number and sms_password
			matches the details in the database */
		$customer = Customer::where('id', '=', $customer_id)->get(array('meter_ID'))->first();
		$scheme = Scheme::where('scheme_number', '=', $scheme_number)->get(array('sms_password'))->first();
		if($scheme->sms_password == $sms_password)
		{
			$message_to_send = urldecode($message);
	...
	public function clear_shutoff($customer_id)
	{
		/* Check if the scheme_number and sms_password
			matches the details in the database */
		$customer = Customer::where('id', '=', $customer_id)->get(array('meter_ID', 'scheme_number'))->first();
		$districtHeating = DistrictHeatingMeter::where('meter_ID', '=', $customer['meter_ID'])->get()->first();
	...
	private function send_customer_sms($customer_id, $scheme_number, $sms_password, $msg_type, $custom_message = '', $charge = true, $replyto = null, $sendOnlyOnce = true)
	{
		...
		$customer = Customer::where('id', '=', $customer_id)->get(array('mobile_number', 'nominated_telephone', 'username'))->first();
		...
		try{
			...
			foreach ($customers as $key => $customer)
			{
				...
				if (strcasecmp($res, "rates") == 0 || strcasecmp($res, "iou") == 0 || strcasecmp($res, "reset") == 0)
				{
					...
					else if (strcasecmp($res, "reset") == 0)
					{
	...
	private function charge_for_sms($scheme_number, $customer_id)
	{
		try
		{
			$sms_charge = Scheme::where('scheme_number', '=', $scheme_number)->get(array('prepage_SMS_charge'))->first();
			$customer = Customer::find($customer_id);
			$customer->balance -= (double)($sms_charge['prepage_SMS_charge']);
			$customer->save();

			return (double)($sms_charge['prepage_SMS_charge']);
		}
		catch(Exception $e)
		{
	...
	private static function replace_keys_in_sms($customer_id, $sms_msg)
	{
		...
		$customer = Customer::where('id', '=', $customer_id)->get(array('scheme_number', 'balance', 'arrears_daily_repayment', 'first_name', 'IOU_available', 'IOU_extra_available'))->first();
		$tarrifs = Tariff::where('scheme_number', '=', $customer['scheme_number'])->get(array('tariff_1', 'tariff_2', 'tariff_3', 'tariff_4', 'tariff_5'))->first();
		$schemes = Scheme::where('scheme_number', '=', $customer['scheme_number'])->get(array('IOU_charge', 'IOU_extra_charge'))->first();
