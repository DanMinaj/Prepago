...
class BillingEngineLogsNew {
	...
	public function parseEntries()
	{
		...
		foreach($this->entries as $e) {
			...
			else if(  (strpos($data, "Applied standing charge") !== false && strpos($data, "New Balance") !== false) ||  (strpos($data, "Applied MISSED standing charge") !== false && strpos($data, "New Balance") !== false) ) {
	...
	public static function getLogRange($customer_id, $from, $to)
	{
		...
		while($from != $to)
		{
			...
			$logs[$from] = new BillingEngineLogsNew($customer_id, $from);
	...
	{
		return new BillingEngineLogsNew($customer_id, $from);
	}
