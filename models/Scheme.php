...
class Scheme extends Eloquent{
	...
	public function refreshFAQ()
	{
		...
		$start_scheme = Scheme::orderBy('scheme_number', 'DESC')->first()->scheme_number;
		
		while(empty($faqs) || !is_array($faqs)) {
			$other_scheme = Scheme::find($start_scheme);
...
->count() == PermanentMeterData::where('scheme_number', $this->scheme_number)->count());		
	...
	public function addFAQ($question, $answer)
	{
		...
		try {
			...
			if(strlen($this->FAQ) <= 0) return false;
	...
	{
		return ($this->status_debug == 0 && $this->archived == 0 && in_array($this->id, Scheme::$un_available_schemes) == false);
	}
	
	public static function active($allow_simulators = true) 
	{
		$un_available_schemes = implode(',', Scheme::$un_available_schemes);
		
		if($allow_simulators == false)
		{
				return Scheme::where('status_debug', 0)->where('archived', 0)
	...
	public function updateSimulator()
	{
		...
		$simulatingScheme = Scheme::find($this->simulator);
		...
		if($dataLogger_simulating) {
			...
			foreach($dataLogger_simulating->getAttributes() as $k => $v) {
				if($k == 'scheme_number' || $k == 'id') continue;
				$dataLogger->$k = $v;
			}
		...
		foreach($customers_to_simulate as $k => $v) {
			...
			foreach($v->getAttributes() as $a => $b) {
				if($a == 'id' || $a == 'simulator' || $a == 'scheme_number' || $a == 'username') continue;
				$customer->$a = $b;
			}	
	...
	$meter_charge = 0.03, $iou_charge = 0, $statements_charge = 0.25, 
	$app_support = 0.25, $vat_number = 'IE9850930S', $company_name = '', $autotopup_charge = 1.95,
	$closed_accounts_charge = 2.00)
	{
	...
	///	$stripe_invoices = StripeCustomerSubscription::APIInvoiceTotal($start_date, $end_date);
		...
		$s->closed_accounts_charge = $closed_accounts_charge;
		$s->iou_charge = $iou_charge;
		$s->statements_charge = $statements_charge;
		$s->amount_of_payments = Scheme::getPaymentAmount($s->scheme_number, $start_date, $end_date);
		$s->value_of_payments = Scheme::getPaymentValue($s->scheme_number, $start_date, $end_date);
		...
		$s->sms_msgs = ( (Scheme::getSMSMessages($s->scheme_number, $start_date, $end_date)) );
		$s->premium_sms_msgs = ( (Scheme::getPremiumSMSMessages($s->scheme_number, $start_date, $end_date)) );
		$s->sms_cost = Scheme::getSMSCost($s->scheme_number);
		$s->premium_sms_cost = Scheme::getPremiumSMSCost($s->scheme_number);
		$s->sms_total_inc_vat = $s->sms_msgs * $s->sms_cost;
		$s->sms_total_ex_vat = ($s->sms_total_inc_vat/$s->remove_vat);
		$s->premium_sms_total_inc_vat = $s->premium_sms_msgs * $s->premium_sms_cost;
		$s->premium_sms_total_ex_vat = ($s->premium_sms_total_inc_vat/$s->remove_vat);
		...
		$s->ious = Scheme::getIOUs($s->scheme_number, $start_date, $end_date);
		$s->iou_charge_inc_vat = ($s->iou_chargeable == 'yes') ? ($s->ious * $s->iou_charge) : 0;
		$s->iou_charge_ex_vat = ($s->iou_charge_inc_vat / $s->remove_vat);
		$s->closed_accounts = Scheme::getClosedAccounts($s->scheme_number, $start_date, $end_date);
		...
		$s->statements_issued = Scheme::getStatements($s->scheme_number, $start_date, $end_date);
		...
		$s->no_of_meters = Scheme::getMeters($s->scheme_number, $start_date, $end_date);
		...
		$s->vat_payment = ($s->cost_of_topups_inc_vat - $s->cost_of_topups_ex_vat) + ($s->sms_total_inc_vat - $s->sms_total_ex_vat) + ($s->app_total_inc_vat - $s->app_total_ex_vat) + ($s->app_support_inc_vat - $s->app_support_ex_vat) + ($s->statements_total_inc_vat - $s->statements_total_ex_vat) + ($s->meter_total_inc_vat - $s->meter_total_ex_vat);
		
		$s->scheme_payment = $s->value_of_payments - ($s->cost_of_topups_inc_vat + $s->sms_total_inc_vat + $s->app_total_inc_vat + $s->app_support_inc_vat + $s->statements_total_inc_vat + $s->meter_total_inc_vat);
		
		$s->total_payment = $s->invoiced_amount + $s->vat_payment;
		$s->avg_daily_kwh = Scheme::getAvgs($s->scheme_number, 'avg_daily_kwh', ['from' => $start_date, 'to' => $end_date]);
		$s->avg_daily_cost = Scheme::getAvgs($s->scheme_number, 'avg_daily_cost', ['from' => $start_date, 'to' => $end_date]);
		$s->annual_avg_kwh_day = Scheme::getAvgs($s->scheme_number, 'annual_avg_kwh_day', ['from' => $start_date, 'to' => $end_date]);
		$s->annual_avg_cost_day = Scheme::getAvgs($s->scheme_number, 'annual_avg_cost_day', ['from' => $start_date, 'to' => $end_date]);
	...
	{
		$scheme = Scheme::where('scheme_number', $scheme_number)->first();
		
		switch($type) {
			case "avg_daily_kwh":
				return Scheme::where('scheme_number', $scheme_number)->first()->avgDailyUsage($params['from'], $params['to']);
			break;
			case "avg_daily_cost":
				return Scheme::where('scheme_number', $scheme_number)->first()->avgDailyCost($params['from'], $params['to']);
			break;
	...
	public static function getSMSCost($scheme_number)
	{
		$scheme = Scheme::where('scheme_number', $scheme_number)->first();
		if(!$scheme)
			return 0;
		
		return $scheme->prepage_SMS_charge;
	}	
	
	public static function getPremiumSMSCost($scheme_number)
	{
		$scheme = Scheme::where('scheme_number', $scheme_number)->first();
		if(!$scheme)
			return 0;
	...
	public function getStatusLogsAttribute()
	{
		...
		try {
			...
			usort($logs, function($a, $b) { return ((new DateTime($a->time)) < (new DateTime($b->time))); 	});
