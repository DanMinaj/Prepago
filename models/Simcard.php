...
class Simcard extends Eloquent{
	...
	public static function reboot($host, $type = 'emnify', $update_status = true, $scheme_id = -1)
	{
		...
		try {
			...
			$SIM = Simcard::where('IP_Address', $host)->first();
			...
			if($type == 'emnify') {
				return Simcard::rebootEmnify($host);
			}
				
			if($type == 'oliviawireless') {
				return Simcard::rebootOlivia($iccid);
			}
			
			if($type == 'eseye') {
				return Simcard::rebootEseye($host, $iccid);
			}
	...
	public static function rebootEmnify($host)
	{
		...
		try {
			...
			$res = Simcard::msg($host, "reboot", true, 10, true);
			$rebooted = false;
			$msg_res = "";
	...
	public static function refreshEmnify($host)
	{
		try {
			...
			$dl->dl_status = $endpoint_status; $dl->save();
	...
	{
		$simcard = Simcard::where('IP_Address', $host)->first();
		if($simcard) {
