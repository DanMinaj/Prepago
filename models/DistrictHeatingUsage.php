...
class DistrictHeatingUsage extends Eloquent{
	...
	public function spreadCharges($days) 
	{
		...
		for($i=$days-1; $i>-1; $i--) {
			...
			$dhu = DistrictHeatingUsage::where('customer_id', $this->customer_id)
	...
	public function getNextDayAttribute()
	{
		...
		$next = DistrictHeatingUsage::where('customer_id', $this->customer_id)
	...
	public function getPrevDayAttribute()
	{
		...
		$prev = DistrictHeatingUsage::where('customer_id', $this->customer_id)
		->where('date', $date)->orderBy('id', 'ASC')->first();
