...
class ProgramController extends BaseController {
	...
	public function manage_schedule()
	{
		...
		try {
			...
			foreach($new_programs_to_add as $p_id => $p) {
				...
				if(empty($new_time)) continue;
				if(empty($new_program)) continue;
				if(empty($new_run_type)) $new_run_type = 0;
				if(empty($new_run_on_weekend_and_holiday)) $new_run_on_weekend_and_holiday = 0;
	...
	public function save_shut_off_engine_settings()
	{
		...
		try {
			...
			if($setting)
			{
			 ...
			 else throw new Exception("Setting # $setting_id does not exist.");
	...
	public function delete_shut_off_engine_settings()
	{
			...
			else throw new Exception("Setting # $setting_id does not exist, therefore cannot delete.");
