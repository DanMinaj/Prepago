...
class WeatherController extends BaseController {
	...
	public function get_topups()
	{
		...
		$result = Weather::whereBetween('date_time', array($from, $to))->where('county', '=', 'Longford')->get();
		...
		foreach($result as $r)
		{
			...
			array_push(
				$data,
				array(date_format(date_create($r->date_time), 'd-M-y'),
	...
	public function csv_topups()
	{
		...
		if($from2 && $to2)
		$result2 = Weather::whereBetween('date_time', array($from2, $to2))->where('county', '=', 'Longford')->get();
	    ...
	    fputcsv($f, array('Weather vs Top Ups - Date Range 1'), $delimiter);
	    fputcsv($f, array('Date', 'Temperature', 'Top Ups', 'Top Ups Amount', 'Top Up Mode', 'Top Up Mean', 'Top Up Median'), $delimiter);
	    foreach ($result as $r) {
	    	$ps = PaymentStorage::where('time_date', 'LIKE', date_format(date_create($r->date_time), 'Y-m-d').'%');
	    	$ps_count = $ps->count();
		...
		if($from2 && $to2){
	    	...
	    	fputcsv($f, array('Weather vs Top Ups - Date Range 2'), $delimiter);
	    	fputcsv($f, array('Date', 'Temperature', 'Top Ups', 'Top Ups Amount'), $delimiter);
	...
	public function get_heatUsage2()
	{
		...
		$result = Weather::whereBetween('date_time', array($from, $to))->where('county', '=', 'Dublin')->get();
	...
	public function csv_heatUsage()
	{
	    ...
	    fputcsv($f, array('Weather vs Heat Usage - Date Range 1'), $delimiter);
	    fputcsv($f, array('Date', 'Temperature', 'Total Usage'), $delimiter);
	    foreach ($result as $r) {
		...
		if($from2 && $to2){
	    	fputcsv($f, array(''), $delimiter);
	    	fputcsv($f, array('Weather vs Heat Usage - Date Range 2'), $delimiter);
	    	fputcsv($f, array('Date', 'Temperature', 'Total Usage'), $delimiter);
		    foreach ($result2 as $r) {
		    	$ps = DistrictHeatingUsage::where('date', '=', date_format(date_create($r->date_time), 'Y-m-d'))->where('scheme_number', '=', 0);
				$kWh = 0;
