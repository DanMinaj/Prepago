	...
	public function adviceNotes()
	{
		...
		if(!empty(Input::get('cover_note'))) {
			...
			foreach($schemes as $k => $s) {
				Scheme::getReportInformation($s, $start_date, $end_date, $vat, $payments_charge, $app_charge, $meter_charge, $iou_charge, $statements_charge, $app_support, $vat_number, $company_name, $autotopup_charge);
		...
		if(!empty(Input::get('pdf'))) {
			...
			if(Input::get('mass') == true) {
				...
				if(Input::get('multi') == 'true') {
					...
					$s = Scheme::getReportInformation($s, $start_date, $end_date, $vat, $payments_charge, $app_charge, $meter_charge, $iou_charge, $statements_charge, $app_support, $vat_number, $company_name, $autotopup_charge);
