...
$dompdf = new Dompdf();
$dompdf->loadView('/var/www/app/views/pdfs/statement.blade.php');
...
Route::get('first_of_mont', function(){
	...
	foreach($aff as $k => $a) {
		...
		foreach($dhu as $k => $d) {
			...
			if($correct_total_usage <= 0) continue;
...
Route::get('cronjobs', function(){
	if ($fh = fopen('/var/www/app/commands/Management/cron_job_manager.log', 'r')) {
		while (!feof($fh)) { $line = fgets($fh); echo nl2br($line); } fclose($fh); 
	}
});
 ...
 *
 */
Route::group(array('before' => 'isInstaller'), function(){
