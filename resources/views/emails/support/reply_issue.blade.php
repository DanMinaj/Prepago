...
<html lang="en-US">
	...
	<body>
		...
		<div>
			...
			@if($owner)
				The issue #{{ $issue->id }} you created has received a new reply.
			@else
				The issue #{{ $issue->id }} you are involved in has received a new reply.
			@endif
			...
			<p style='background: #ffffff; padding: 20px; /* border-radius: 2px; */ color: #000000; border-top: 1px solid #636363;'>
				...
				<br/><br/><br/>
				<i>{{ $reply->operator }} - {{ $reply->operator_email }}</i>
			</p>
